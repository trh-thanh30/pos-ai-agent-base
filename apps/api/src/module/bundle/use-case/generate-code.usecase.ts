...
@Injectable()
export class GenerateBundleSkuUseCase implements IGenerateSkuUseCase {
  private prefix = 'CB';
  private padLength = 5;
  ...
  async generateSkuBatch(storeId: string, count: number): Promise<string[]> {
    ...
    const products = await this.prisma.bundle.findMany({
      where: { storeId: storeId, sku: { startsWith: this.prefix } },
      select: { sku: true },
    });
  ...
  async generateSkuBatchWithTransaction(
    ...
    const products = await tx.bundle.findMany({
      where: { storeId: storeId, sku: { startsWith: this.prefix } },
      select: { sku: true },
    });
