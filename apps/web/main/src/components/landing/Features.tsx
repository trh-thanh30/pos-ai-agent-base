...
const metrics = [
  ...
  { value: '99.9%', label: 'hoạt động ổn định liên tục' },
];
        ...
        <div className="flex flex-col justify-between h-full">
          <div>
            <SectionHeader
              ...
              description="NexPOS được thiết kế cho người dùng thao tác mỗi ngày: chủ cửa hàng, thu ngân, quản lý kho và đội vận hành chuỗi."
            />
            ...
            <div className="mt-8 flex flex-col gap-3">
              {metrics.map((item) => (
                <div
                  key={item.label}
                  className="rounded-2xl border border-slate-100 bg-slate-50 p-5 shadow"
                >
                  <div className="text-3xl font-bold text-slate-950">{item.value}</div>
                  <p className="mt-1.5 text-sm font-medium text-slate-600">{item.label}</p>
                </div>
              ))}
            </div>
          </div>
